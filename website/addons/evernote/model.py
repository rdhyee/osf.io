# -*- coding: utf-8 -*-
"""Persistence layer for the evernote addon.
"""
...
class AddonEvernoteUserSettings(AddonUserSettingsBase):
...
class AddonEvernoteNodeSettings(AddonNodeSettingsBase):

    user_settings = fields.ForeignField(
        'evernoteusersettings', backref='authorized'
    )
    ...
    def deauthorize(self, auth):
        ...
        self.owner.add_log(
            action='evernote_node_deauthorized',
            params={
    ...
    def before_remove_contributor_message(self, node, removed):
        ...
        who authorized the Evernote addon
        """
    ...
    def after_register(self, node, registration, user, save=True):
        ...
        """
        clone, message = super(EvernoteNodeSettings, self).after_register(
    ...
    def after_fork(self, node, fork, user, save=True):
        ...
        :return: A tuple of the form (cloned_settings, message)
        """
        clone, _ = super(EvernoteNodeSettings, self).after_fork(
            node=node, fork=fork, user=user, save=False
        )

        if self.user_settings and self.user_settings.owner == user:
            clone.user_settings = self.user_settings
            message = 'Evernote authorization copied to fork.'
        else:
            message = ('Evernote authorization not copied to fork. You may '
    ...
    def after_remove_contributor(self, node, removed):
        """If the removed contributor was the user who authorized the Evernote
        ...
        if self.user_settings and self.user_settings.owner == removed:
            ...
            return ('Because the Evernote add-on for this project was authenticated'
