...
def evernote_config_get(node_addon, **kwargs):
...
@must_have_addon('evernote', 'node')
def evernote_config_put(node_addon, auth, **kwargs):
...
@must_have_permission('write')
@must_have_addon('evernote', 'node')
def evernote_deauthorize(auth, node_addon, **kwargs):
...
@must_have_permission('write')
@must_have_addon('evernote', 'node')
def evernote_import_user_auth(auth, node_addon, **kwargs):
    """Import evernote credentials from the currently logged-in user to a node.
    """
    user = auth.user
    user_addon = user.get_addon('evernote')
